...
// Department to image mapping (using custom ECR workspace images)
const DEPARTMENT_IMAGES = {
  ...
  'default': `${ECR_REGISTRY}/workspace-base:latest`
};
...
async function provisionWorkspace(employee) {
  if (!initK8sClient()) {
    logger.warn('Kubernetes client not available - using mock provisioning');
    return await mockProvisionWorkspace(employee);
  }
  ...
  try {
    // 1. Ensure namespace exists (30s timeout)
    await Promise.race([
      ensureNamespace(),
      new Promise((_, reject) => setTimeout(() => reject(new Error('Kubernetes API timeout (namespace check)')), 30000))
    ]);

    // 2. Create Secret for VNC password
    await Promise.race([
      createWorkspaceSecret(workspaceName, vncPassword, employee),
      new Promise((_, reject) => setTimeout(() => reject(new Error('Kubernetes API timeout (secret)')), 30000))
    ]);
    logger.info(`Secret created for ${workspaceName}`);

    // 3. Create PVC for workspace data
    await Promise.race([
      createWorkspacePVC(workspaceName, department),
      new Promise((_, reject) => setTimeout(() => reject(new Error('Kubernetes API timeout (PVC)')), 30000))
    ]);
    logger.info(`PVC created for ${workspaceName}`);

    // 4. Create Kasm workspace Pod
    await Promise.race([
      createKasmPod(workspaceName, employee, department, workspaceId),
      new Promise((_, reject) => setTimeout(() => reject(new Error('Kubernetes API timeout (pod)')), 30000))
    ]);
    logger.info(`Kasm pod created for ${workspaceName}`);

    // 5. Create Service for workspace access
    await Promise.race([
      createWorkspaceService(workspaceName, employee.employeeId),
      new Promise((_, reject) => setTimeout(() => reject(new Error('Kubernetes API timeout (service)')), 30000))
    ]);
    logger.info(`Service created for ${workspaceName}`);

    // 6. Wait for pod to be ready and get NodePort + Node IP (90s for pod to start + image pull)
    const { nodePort, nodeIp } = await waitForWorkspaceReady(workspaceName, 90);
    ...
    // If Kubernetes provisioning fails, try mock provisioning as fallback
    if (error.message.includes('timeout') || 
        error.message.includes('ECONNREFUSED') || 
        error.message.includes('ETIMEDOUT') ||
        error.message.includes('did not become ready') ||
        error.message.includes('Kubernetes API')) {
      logger.warn('Kubernetes provisioning failed - using mock provisioning as fallback');
      try {
        return await mockProvisionWorkspace(employee);
      } catch (mockError) {
        logger.error('Mock provisioning also failed:', mockError);
      }
    }
...
/**
 * Mock workspace provisioning for demo/development when K8s is unavailable
 */
async function mockProvisionWorkspace(employee) {
  const workspaceId = uuidv4();
  const department = getDepartment(employee);
  const vncPassword = generateSecurePassword();
  const firstName = employee.firstName?.toLowerCase() || 'user';
  const lastName = employee.lastName?.toLowerCase().replace(/\s+/g, '') || 'workspace';
  
  logger.info(`Creating mock workspace for ${employee.employeeId}`);
  
  // Create mock workspace with demo URL
  const workspace = {
    workspaceId,
    employeeId: employee.employeeId,
    name: `ws-${workspaceId}`,
    department,
    url: `https://${firstName}.${lastName}.innovatech.local:30098`,
    dnsName: `${firstName}.${lastName}.innovatech.local`,
    nodeIp: '10.0.83.236',
    nodePort: 30098,
    vncPort: 6901,
    status: 'active',
  ...
  await dynamodbService.createWorkspace(workspace);
  logger.info(`Mock workspace created for ${employee.employeeId}`);
  
  // Try to store password in SSM
  try {
    await ssmService.storeTemporaryPassword(employee.employeeId, vncPassword);
  } catch (ssmError) {
    logger.warn(`Failed to store password in SSM: ${ssmError.message}`);
...
async function createKasmPod(name, employee, department, workspaceId) {
  ...
  const pod = {
    ...
    metadata: {
      ...
      labels: {
        app: 'workspace',
        department: sanitizeK8sName(department),
        employee: sanitizeK8sName(employee.employeeId),
