...
async function getDirectoryConfig() {
  try {
    ...
    // Get DNS servers (required for AD join and LDAP)
    let dnsServers = null;
    let dnsIps = null;
    ...
    try {
      const dnsIpsParam = await ssmClient.send(new GetParameterCommand({
        Name: `/${CLUSTER_NAME}/directory/dns-ips`
      }));
      dnsIps = dnsIpsParam.Parameter.Value;
    } catch (e) {
      logger.warn('Directory DNS IPs not found in SSM');
    }
