...
resource "aws_instance" "nat" {
  ...
  # User data to configure NAT - using shell script format for AL2023
  user_data = <<-EOF
#!/bin/bash
set -ex
...
echo "net.ipv4.ip_forward = 1" >> /etc/sysctl.d/10-nat.conf

# Get interface
INTERFACE=$(ip -4 route ls | grep default | awk '{print $5}')
...
echo "NAT configuration complete at $(date)"
